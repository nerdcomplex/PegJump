...
-(void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    if (![game started]) {
        [game.recorder movePeg:[game.data copy] :[NSArray arrayWithObjects:
                                                  [NSNumber numberWithInt:self.row],[NSNumber numberWithInt:self.col], nil]];
        [[game data] flipLoc:self.row :self.col];
        [game setStarted:YES];
        [game startTimer];
    }
    else if ([[game possibleMoves] count]==0) {
        [game setCurrentPeg:[NSArray arrayWithObjects:[NSNumber numberWithInt:self.row],
                             [NSNumber numberWithInt:self.col], nil]];
        ...
        [game.recorder movePeg:[game.data copy] :[game currentPeg]];
        
        [[game data] flipLoc:self.row :self.col];
        [[game data] flipLoc:[[[game currentPeg] objectAtIndex:0] intValue] :[[[game currentPeg] objectAtIndex:1] intValue]];
        [[game data] flipLoc:([[[game currentPeg] objectAtIndex:0] intValue]+self.row)/2
                            :([[[game currentPeg] objectAtIndex:1] intValue]+self.col)/2];
